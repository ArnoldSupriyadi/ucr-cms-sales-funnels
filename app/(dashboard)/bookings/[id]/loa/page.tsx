...
export default async function LoaPage({
  ...
  const { data: booking } = await supabase
    .from('bookings')
    .select('id, booking_no, event_name')
    .eq('id', id)
    .single()
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Letter of Agreement</h1>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-slate-500">
            Fitur LoA akan dibangun pada Bulan 3. Termasuk form multi-item, kalkulasi
            harga otomatis, approval flow GM via WA/Telegram, dan generate PDF.
          </p>
        </CardContent>
      </Card>
    </div>
