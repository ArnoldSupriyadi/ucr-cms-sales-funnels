...
interface PackageComboboxProps {
  ...
  /** Node portal popover (mis. body Sheet) agar wheel-scroll tak diblokir RemoveScroll. */
  container?: HTMLElement | null
}

/** Picker paket ber-search (Popover + Command). Dipakai utk "Tambah dari Paket". */
export function PackageCombobox({ packagesByKategori, value, onChange, container }: PackageComboboxProps) {
  const [open, setOpen] = React.useState(false)
  const selected = packagesByKategori.flatMap(([, pkgs]) => pkgs).find((p) => p.id === value)
