...
CREATE TABLE bookings (
  ...
  sales_id                UUID REFERENCES users(id) ON DELETE SET NULL,
  status                  booking_status_enum NOT NULL DEFAULT 'Tentative',
...
COMMENT ON TABLE bookings IS 'Event bookings. Status flow: Tentative→Definite→Actual (terminal) or Cancel (terminal).';
COMMENT ON COLUMN bookings.booking_no IS 'Format: UCR-YYYYMM-XXX. Generated by app on creation.';
COMMENT ON COLUMN bookings.is_exception IS 'True if booking was made outside normal SOP (e.g. below minimum pax).';
...
COMMENT ON TABLE booking_status_logs IS 'Immutable audit log of all booking status changes.';
...
COMMENT ON TABLE loa IS 'Letter of Agreement. 1:1 with booking. net_revenue = sub_total_2, used as IB revenue base.';
...
COMMENT ON TABLE ib IS 'Internal Breakdown (P&L per booking). Created by Cost Controller only, after LoA status=final.';
...
CREATE INDEX idx_bookings_sales_id   ON bookings(sales_id);
CREATE INDEX idx_bookings_status     ON bookings(status);
CREATE INDEX idx_bookings_event_date ON bookings(event_date);
CREATE INDEX idx_bookings_segmen     ON bookings(segmen);

-- Booking status logs
CREATE INDEX idx_bsl_booking_id ON booking_status_logs(booking_id);
...
-- ---- BOOKINGS: sales sees own, admin/GM/CC sees all ----
CREATE POLICY "bookings_select"
  ON bookings FOR SELECT
  TO authenticated
  USING (sales_id = auth.uid() OR is_admin_or_gm() OR get_my_role() = 'Cost Controller');

CREATE POLICY "bookings_insert_sales"
...
-- ---- BOOKING_STATUS_LOGS: read follows booking, write by authenticated ----
CREATE POLICY "bsl_select"
  ...
  USING (
    EXISTS (
      ...
      WHERE b.id = booking_status_logs.booking_id
        AND (b.sales_id = auth.uid() OR is_admin_or_gm() OR get_my_role() = 'Cost Controller')
    )
  );

CREATE POLICY "bsl_insert"
  ON booking_status_logs FOR INSERT
  TO authenticated
  WITH CHECK (true);
...
INSERT INTO roles (name, permissions, is_default) VALUES
  ('Super Admin', '{
    ...
    "bookings.create": true, "bookings.view_all": true, "bookings.edit": true,
  ...
  ('GM', '{
    ...
    "bookings.create": false, "bookings.view_all": true, "bookings.edit": false,
  ...
  ('Sales', '{
    ...
    "bookings.create": true, "bookings.view_all": false, "bookings.edit": true,
